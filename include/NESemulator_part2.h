...
const mapintf_t *mappers[] = {
    ...
    &map15_intf, &map16_intf, &map18_intf, &map19_intf,  &map21_intf,
    &map22_intf, &map23_intf, &map24_intf, &map25_intf,  &map32_intf,
    &map33_intf, &map34_intf, &map40_intf, &map64_intf,  &map65_intf,
    &map66_intf, &map70_intf, &map75_intf, &map78_intf,  &map79_intf,
...
void nes_reset() {
  ...
  /// if (nes.rominfo->vram) memset(nes.rominfo->vram, 0, 0x2000 *
  /// nes.rominfo->vram_banks);
...
nes_t *nes_create(void) {
  ...
  /// if (NULL != machine->cpu->mem_page[0]) memset(machine->cpu->mem_page[0],
  /// 0, sizeof(NES_RAMSIZE));
  ...
  if (SOUND_ENABLED) {
    ...
    machine->apu =
        apu_create(0, osd_sound.sample_rate, NES_REFRESH_RATE, osd_sound.bps);
...
static nes6502_memwrite default_writehandler[] = {
    {0x0800, 0x1FFF, ram_write},     {0x2000, 0x3FFF, ppu_write},
...
static void build_address_handlers(nes_t *machine) {
  ...
  if (intf->sound_ext && SOUND_ENABLED) {
    if (NULL != intf->sound_ext->mem_read) {
      for (count = 0; num_handlers < MAX_MEM_HANDLERS;
           count++, num_handlers++) {
        if (NULL == intf->sound_ext->mem_read[count].read_func) break;
        memcpy(&machine->readhandler[num_handlers],
               &intf->sound_ext->mem_read[count], sizeof(nes6502_memread));
      }
    }
  }
  ...
  if (intf->sound_ext) {
    if (NULL != intf->sound_ext->mem_write) {
      for (count = 0; num_handlers < MAX_MEM_HANDLERS;
        ...
        memcpy(&machine->writehandler[num_handlers],
               &intf->sound_ext->mem_write[count], sizeof(nes6502_memwrite));
      }
    }
  }
