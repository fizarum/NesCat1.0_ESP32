...
map_memwrite map1_memwrite[] = {{0x8000, 0xFFFF, map1_write}, {-1, -1, NULL}};
...
map_memwrite map2_memwrite[] = {{0x8000, 0xFFFF, map2_write}, {-1, -1, NULL}};
...
map_memwrite map3_memwrite[] = {{0x8000, 0xFFFF, map3_write}, {-1, -1, NULL}};
...
map_memwrite map4_memwrite[] = {{0x8000, 0xFFFF, map4_write}, {-1, -1, NULL}};
  ...
  switch (address) {
    ...
    case MMC5_WRB2:
      ...
      if (mmc5.rect[chan].enabled)
        mmc5.rect[chan].freq =
            (((mmc5.rect[chan].regs[3] & 7) << 8) + value) + 1;
      break;
    ...
    case MMC5_WRB3:
      ...
      if (mmc5.rect[chan].enabled) {
        ...
        mmc5.rect[chan].freq =
            (((value & 7) << 8) + mmc5.rect[chan].regs[2]) + 1;
...
apu_memread mmc5_memread[] = {{0x5205, 0x5206, mmc5_read}, {-1, -1, NULL}};

apu_memwrite mmc5_memwrite[] = {{0x5000, 0x5015, mmc5_write},
...
void map5_setstate(SnssMapperBlock *state) { UNUSED(state); }
...
map_memread map5_memread[] = {{0x5204, 0x5204, map5_read}, {-1, -1, NULL}};
...
void map7_init(void) { mmc_bankrom(32, 0x8000, 0); }

map_memwrite map7_memwrite[] = {{0x8000, 0xFFFF, map7_write}, {-1, -1, NULL}};
...
map_memwrite map8_memwrite[] = {{0x8000, 0xFFFF, map8_write}, {-1, -1, NULL}};
...
void map9_write(uint32_t address, uint8_t value) {
  switch ((address & 0xF000) >> 12) {
    ...
    case 0xB:
      regs9[0] = value;
      if (0xFD == latch9[0]) mmc_bankvrom(4, 0x0000, value);
      break;

    case 0xC:
      regs9[1] = value;
      if (0xFE == latch9[0]) mmc_bankvrom(4, 0x0000, value);
      break;

    case 0xD:
      regs9[2] = value;
      if (0xFD == latch9[1]) mmc_bankvrom(4, 0x1000, value);
      break;

    case 0xE:
      regs9[3] = value;
      if (0xFE == latch9[1]) mmc_bankvrom(4, 0x1000, value);
      break;
...
map_memwrite map9_memwrite[] = {{0x8000, 0xFFFF, map9_write}, {-1, -1, NULL}};
...
map_memwrite map11_memwrite[] = {{0x8000, 0xFFFF, map11_write}, {-1, -1, NULL}};
...
void map15_init(void) { mmc_bankrom(32, 0x8000, 0); }

map_memwrite map15_memwrite[] = {{0x8000, 0xFFFF, map15_write}, {-1, -1, NULL}};
...
map_memwrite map16_memwrite[] = {{0x6000, 0x600D, map16_write},
...
#define VRC_PBANK(bank, value, high)                                    \
  do {                                                                  \
    ...
    mmc_bankrom(8, 0x8000 + ((bank) << 13),                             \
                (highprgnybbles[(bank)] << 4) + lowprgnybbles[(bank)]); \
  } while (0)
...
#define VRC_VBANK(bank, value, high)                               \
  {                                                                \
    ...
    mmc_bankvrom(1, (bank) << 10,                                  \
                 (highnybbles[(bank)] << 4) + lownybbles[(bank)]); \
  }
...
void map18_init(void) { irq18.counter = irq18.enabled = 0; }
...
map_memwrite map18_memwrite[] = {{0x8000, 0xFFFF, map18_write}, {-1, -1, NULL}};
  ...
  {                                                                           \
    if ((value) < 0xE0)                                                       \
      ppu_setpage(                                                            \
          1, (table) + 8,                                                     \
          &mmc_getinfo()                                                      \
                  ->vrom[((value) % (mmc_getinfo()->vrom_banks * 8)) << 10] - \
              (0x2000 + ((table) << 10)));                                    \
    else                                                                      \
      ppu_setpage(1, (table) + 8,                                             \
                  &mmc_getinfo()->vram[((value)&7) << 10] -                   \
                      (0x2000 + ((table) << 10)));                            \
    ppu_mirrorhipages();                                                      \
  }
...
void map19_init(void) { irq19.counter = irq19.enabled = 0; }
...
map_memwrite map19_memwrite[] = {{0x5000, 0x5FFF, map19_write},
  ...
  switch (address & 0xB003) {
    ...
    case 0xA001:
      ...
      vrcvi.rectangle[chan].freq =
          ((vrcvi.rectangle[chan].reg[2] & 0x0F) << 8) + value + 1;
      break;
    ...
    case 0xA002:
      ...
      vrcvi.rectangle[chan].freq =
          ((value & 0x0F) << 8) + vrcvi.rectangle[chan].reg[1] + 1;
...
apu_memwrite vrcvi_memwrite[] = {{0x9000, 0x9002, vrcvi_write}, /* vrc6 */
...
void map24_write(uint32_t address, uint8_t value) {
  switch (address & 0xF003) {
    ...
    case 0xF001:
      ...
      if (irq24.enabled) irq24.counter = irq24.latch;
      break;
...
map_memwrite map24_memwrite[] = {{0x8000, 0xF002, map24_write}, {-1, -1, NULL}};
...
map_memwrite map32_memwrite[] = {{0x8000, 0xFFFF, map32_write}, {-1, -1, NULL}};
...
map_memwrite map33_memwrite[] = {{0x8000, 0xFFFF, map33_write}, {-1, -1, NULL}};
...
//................................................................................
// MAPPER34
void map34_init(void) { mmc_bankrom(32, 0x8000, MMC_LASTBANK); }
...
map_memwrite map34_memwrite[] = {{0x7FFD, 0xFFFF, map34_write}, {-1, -1, NULL}};
...
map_memwrite map40_memwrite[] = {{0x8000, 0xFFFF, map40_write}, {-1, -1, NULL}};
...
map_memwrite map41_memwrite[] = {{0x6000, 0x67FF, map41_low_write},
...
map_memwrite map42_memwrite[] = {{0xE000, 0xFFFF, map42_write}, {-1, -1, NULL}};
...
map_memwrite map46_memwrite[] = {{0x6000, 0xFFFF, map46_write}, {-1, -1, NULL}};
...
map_memwrite map50_memwrite[] = {{0x4000, 0x5FFF, map50_write}, {-1, -1, NULL}};
...
void map64_write(uint32_t address, uint8_t value) {
  ...
  if (true == irq64.reset) irq64.counter = irq64.latch;
}
...
map_memwrite map64_memwrite[] = {{0x8000, 0xFFFF, map64_write}, {-1, -1, NULL}};
...
map_memwrite map65_memwrite[] = {{0x8000, 0xFFFF, map65_write}, {-1, -1, NULL}};
...
map_memwrite map66_memwrite[] = {{0x8000, 0xFFFF, map66_write}, {-1, -1, NULL}};
...
map_memwrite map70_memwrite[] = {{0x8000, 0xFFFF, map70_write}, {-1, -1, NULL}};
...
map_memwrite map73_memwrite[] = {{0x8000, 0xFFFF, map73_write}, {-1, -1, NULL}};
...
map_memwrite map75_memwrite[] = {{0x8000, 0xFFFF, map75_write}, {-1, -1, NULL}};
...
map_memwrite map78_memwrite[] = {{0x8000, 0xFFFF, map78_write}, {-1, -1, NULL}};
  ...
  switch (bank) {
    ...
    case 0x0F:
      if (0x10 == reg) {
        ...
        if (true == irq85.enabled) irq85.counter = irq85.latch;
      }
      break;
...
map_memwrite map85_memwrite[] = {{0x8000, 0xFFFF, map85_write}, {-1, -1, NULL}};
...
map_memwrite map87_memwrite[] = {{0x6000, 0x7FFF, map87_write}, {-1, -1, NULL}};
...
map_memwrite map93_memwrite[] = {{0x8000, 0xFFFF, map93_write}, {-1, -1, NULL}};
...
map_memwrite map94_memwrite[] = {{0x8000, 0xFFFF, map94_write}, {-1, -1, NULL}};
...
map_memwrite map160_memwrite[] = {{0x8000, 0xFFFF, map160_write},
                                  {-1, -1, NULL}};
...
map_memwrite map229_memwrite[] = {{0x8000, 0xFFFF, map229_write},
                                  {-1, -1, NULL}};
...
void map231_init(void) { mmc_bankrom(32, 0x8000, MMC_LASTBANK); }
...
map_memwrite map231_memwrite[] = {{0x8000, 0xFFFF, map231_write},
                                  {-1, -1, NULL}};
...
map_memwrite map21_memwrite[] = {{0x8000, 0xFFFF, map21_write}, {-1, -1, NULL}};

map_memwrite map22_memwrite[] = {{0x8000, 0xFFFF, map22_write}, {-1, -1, NULL}};

map_memwrite map23_memwrite[] = {{0x8000, 0xFFFF, map23_write}, {-1, -1, NULL}};
