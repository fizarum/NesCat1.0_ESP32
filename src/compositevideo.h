...
static inline void IRAM_ATTR i2s_intr_handler_video(void* arg) {
  if (I2S0.int_st.out_eof)
    video_isr(
        ((lldesc_t*)I2S0.out_eof_des_addr)->buf);  // get the next line of video
...
static esp_err_t start_dma(int line_width, int samples_per_cc, int ch = 1) {
  ...
  buffer
  ///    dma_descriptor[buff_idx].size = sizeof(dma_buff[buff_idx]); //max size
  of the buffer in bytes dma_descriptor[buff_idx].size = 2000;  //max size of
  the buffer in bytes dma_descriptor[buff_idx].owner = 1;
      ...
      if (buff_idx==49) dma_descriptor[buff_idx].qe.stqe_next =
  &dma_descriptor[0]; else dma_descriptor[buff_idx].qe.stqe_next =
  &dma_descriptor[buff_idx+1];
  }
  ...
  /*
    // Create RX DMA buffers
      ...
      dma_descriptor[0].size = sizeof(dma_buff[0]); //max size of the buffer in
  bytes dma_descriptor[0].owner = 1; dma_descriptor[0].sosf = 1;
      ...
      dma_descriptor[1].size = sizeof(dma_buff[1]); //max size of the buffer in
  bytes dma_descriptor[1].owner = 1; dma_descriptor[1].sosf = 1;
      ...
      dma_descriptor[2].size = sizeof(dma_buff[2]); //max size of the buffer in
  bytes dma_descriptor[2].owner = 1; dma_descriptor[2].sosf = 1;
      ...
      dma_descriptor[3].size = sizeof(dma_buff[3]); //max size of the buffer in
  bytes dma_descriptor[3].owner = 1; dma_descriptor[3].sosf = 1;
  ...
  */
  ...
  //  f_out = xtal_freq * (4 + sdm2 + sdm1/256 + sdm0/65536); // 250 < f_out <
  //  500 apll_freq = f_out/((o_div + 2) * 2) operating range of the f_out is
  //  250 MHz ~ 500 MHz operating range of the apll_freq is 16 ~ 128 MHz. select
  //  sdm0,sdm1,sdm2 to produce nice multiples of colorburst frequencies
  ...
  if (!_pal_) {
    switch (samples_per_cc) {
      ...
      case 4:
        rtc_clk_apll_enable(1, 70, 151, 4, 1);
  ...
  ///      rtc_clk_apll_enable(1, 0x46, 0x97, 0x4, 1);   // 14.3181818182 4x
  ...
  /// APL_CLK = 40 * (4 + (sdm0 / 65536) + (sdm1 / 256) + sdm2) / (2 * (o_div +
  /// 2));  // final freq /4
  //-------------------------------------------------------------------------------
  //===============================================================================
  ...
  I2S0.clkm_conf.clkm_div_a =
      1;  // Fractional clock divider’s denominator value
          ///  I2S0.sample_rate_conf.tx_bck_div_num = 1;

  //===============================================================================
  ///  I2S0.sample_rate_conf.rx_bck_div_num = 2;  /// WE HAVE DOUBLED FREQ SO
  ///  MUST DIVIDE BY 2
  I2S0.sample_rate_conf.tx_bck_div_num =
      1;  /// WE HAVE DOUBLED FREQ SO MUST DIVIDE BY 2
  //===============================================================================

  I2S0.clkm_conf.clka_en = 1;  // Set this bit to enable clk_apll.
  I2S0.fifo_conf.tx_fifo_mod =
      (ch == 2) ? 0 : 1;  // 32-bit dual or 16-bit single channel data

  ///  I2S0.fifo_conf.rx_fifo_mod = (ch == 2) ? 0 : 1; // 32-bit dual or 16-bit
  ///  single channel data
  ...
  ///  I2S0.conf.rx_start = 1;                     // start DMA! for
  ///  OSCILLOSCOPE
